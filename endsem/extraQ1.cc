...
main (int argc, char *argv[])
{
  ...
  Ipv4InterfaceContainer p2p_interface_3;
  p2p_interface_3 = address.Assign (p2p_devices_56);


  UdpEchoServerHelper echoServer (9);

  ApplicationContainer server_apps = echoServer.Install (p2.Get (1));
  server_apps.Start (Seconds (1.0));
  server_apps.Stop (Seconds (10.0));

  UdpEchoClientHelper echoClient (p2p_interface_3.GetAddress (1), 9);
  ...
  AnimationInterface anim ("AAextraQ1.xml");
  Simulator::Run ();
  Simulator::Destroy ();
  return 0;
}
